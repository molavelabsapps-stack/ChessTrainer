    ...
    .map((row, displayRow) => {
      ...
      return cols
        .map((piece, displayCol) => {
          ...
          return `<button class="square ${light ? "light" : "dark"} ${selected ? "is-selected" : ""}" data-row="${actualRow}" data-col="${actualCol}" aria-label="${squareName(actualRow, actualCol)}">${pieceImg}${coord}</button>`;
        })
        .join("");
    })
    .join("");
